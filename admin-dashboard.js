...
function renderRate() {
  ...
  view.innerHTML = `
    ...
    <p class="hint" style="margin-bottom:16px">چون این کانال عمومی است، نیازی به ساختن ربات نیست. کافیست فایل <code>server/fetch_rate.php</code> روی هاست واقعی سایت با یک Cron Job هر چند دقیقه اجرا شود؛ بعد از آن، این صفحه و کل سایت خودکار نرخ را از فایل <code>server/rate.json</code> می‌خوانند. راهنمای کامل گام‌به‌گام در فایل <code>server/README-telegram.md</code> است.</p>
    ...
    <div style="display:flex;gap:10px">
      ...
      <button type="button" class="btn btn-wine" id="fetch-now">همگام‌سازی نرخ از سرور (server/rate.json)</button>
    </div>
  ...
  document.getElementById('fetch-now').addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('../server/rate.json', { cache: 'no-store' });
      if (res.ok) {
        const data = await res.json();
        if (data && data.rate) {
          saveSettings({ ratePerGram: data.rate, lastRateUpdate: data.updatedAt || new Date().toISOString() });
          resEl.style.color = '#8fd08f';
          resEl.textContent = `موفق: نرخ ${formatToman(data.rate)} از فایل server/rate.json (تولیدشده توسط Cron Job) خوانده و ثبت شد.`;
          renderRate();
          return;
        }
      }
      throw new Error('فایل server/rate.json هنوز موجود نیست (یعنی سایت هنوز روی سرور واقعی با Cron Job مستقر نشده).');
    } catch (err) {
      resEl.style.color = '#e79999';
      resEl.textContent = 'خطا: ' + err.message + ' — تا وقتی سایت به سرور واقعی وصل نشده، نرخ رو دستی وارد کن. راهنمای کامل در server/README-telegram.md است.';
    }
  });
}
