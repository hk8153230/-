if (!isAdminLoggedIn()) location.href = 'login.html';
...
function renderRate() {
  ...
  view.innerHTML = `
    ...
    <h3 style="font-size:16px;margin-bottom:6px">دریافت خودکار نرخ از کانال تلگرام اتحادیه</h3>
    <p class="hint" style="margin-bottom:16px">اگر لینک کانال اتحادیه را داری، آدرسش رو اینجا ذخیره کن؛ برای خودکارسازی کامل هم مراحل زیر رو طبق راهنما انجام بده (راهنمای گام‌به‌گام در فایل <code>server/README-telegram.md</code> داخل پوشه‌ی سایت هست).</p>
    <form id="tg-form" style="max-width:520px">
      <div class="field"><label>لینک کانال تلگرام اتحادیه</label><input name="telegramChannelLink" placeholder="https://t.me/..." value="${s.telegramChannelLink || ''}"></div>
      <div class="field"><label>توکن ربات (Bot Token)</label><input name="telegramBotToken" placeholder="123456:ABC-..." value="${s.telegramBotToken || ''}"></div>
      <div class="field"><label>آیدی/یوزرنیم کانال (اختیاری)</label><input name="telegramChatId" placeholder="@channelusername" value="${s.telegramChatId || ''}"></div>
      <button class="btn btn-outline" type="submit">ذخیره تنظیمات تلگرام</button>
      <button type="button" class="btn btn-wine" id="fetch-now" style="margin-inline-start:8px">دریافت نرخ الان (آزمایشی)</button>
    </form>
  ...
  document.getElementById('tg-form').addEventListener('submit', e => {
    ...
    const fd = new FormData(e.target);
    saveSettings({
      telegramChannelLink: fd.get('telegramChannelLink'),
      telegramBotToken: fd.get('telegramBotToken'),
      telegramChatId: fd.get('telegramChatId')
    });
    toastAdmin('تنظیمات تلگرام ذخیره شد');
  });
  ...
  document.getElementById('fetch-now').addEventListener('click', async () => {
    ...
    try {
      ...
      const result = await tryFetchTelegramRate(s2.telegramBotToken, s2.telegramChatId, null);
      saveSettings({ ratePerGram: result.rate, lastRateUpdate: new Date().toISOString() });
      resEl.style.color = '#8fd08f';
      resEl.textContent = `موفق: نرخ ${formatToman(result.rate)} از آخرین پیام کانال دریافت و ثبت شد.`;
      renderRate();
    } catch (err) {
      resEl.style.color = '#e79999';
      resEl.textContent = 'خطا: ' + err.message + ' — راهنمای اتصال کامل در server/README-telegram.md است، یا فعلاً نرخ رو دستی وارد کن.';
    }
  });
}
